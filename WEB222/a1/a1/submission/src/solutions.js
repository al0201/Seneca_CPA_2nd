...
/*******************************************************************************
 * Problem 0: learn how to implement conpmde to pass unit tests.
 *
...
function createLinkTag(rel, href) {
  let trimmedRel = rel.trim();
  let trimmedHref = href.trim();

  const specialChars = /[ \t\n\r]/;

  if (specialChars.test(trimmedRel) || specialChars.test(trimmedHref)) {
    trimmedRel = `"${trimmedRel}"`;
    trimmedHref = `"${trimmedHref}"`;
  }

  let linkElement = `<link rel=${trimmedRel} href=${trimmedHref}>`;
  return linkElement;
}
...
function parseDateString(value) {
  if (typeof value !== 'string') {
    throw new Error('Invalid input: expected a string in `YYYY/MM/DD` or `DD/MM/YYYY` format.');
  }
  ...
  const matchedFormat = dateFormats.find((format) => format.regex.test(value));
  ...
  const match = value.match(matchedFormat.regex);
  const year = parseInt(match[matchedFormat.yearIndex], 10);
  const month = parseInt(match[matchedFormat.monthIndex], 10) - 1;
  const day = parseInt(match[matchedFormat.dayIndex], 10);
...
function toDateString(value, format) {
  ...
  if (!validFormats.includes(format)) {
    throw new Error('Invalid format: expected `YYYY/MM/DD`, `DD/MM/YYYY`, or `MM/DD/YYYY`.');
  }
  ...
  switch (format) {
    ...
    default:
      throw new Error('Invalid format: unexpected format string.');
  }
}
...
function normalizeTime(value) {
  const timeRegex = /^(0?[1-9]|1[0-2]):([0-5][0-9]):([0-5][0-9])\s?(AM|PM)?$/;
  const militaryTimeRegex = /^(0?[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$/;
  ...
  if (match) {
    ...
    if (
      ...
      return null; // Invalid time format, return null
    }
...
function buildApiEndpoint(resourceType, resourceId, extraInfo) {
  ...
  if (extraInfo !== undefined && extraInfo.length === 0) {
    throw new Error('Invalid parameter: extraInfo cannot be an empty string.');
  }

  let endpoint = `/${resourceType}/${resourceId}`;

  if (extraInfo !== undefined && extraInfo.length > 0) {
    let encodedExtraInfo = encodeURIComponent(extraInfo);
    endpoint += `/${encodedExtraInfo}`;
  }
